...
class GetData():
    ...
    def extract_nucl_data(self):
        try:
            p=self.extracted_path
            return str(self.path.parent / p)
        except:
            if self.path.exists():
                print(f'Info: extracting nuclear cross sections into {self.dirname}')
                results=sp.run(['tar','--verbose','--directory',self.dirname,'-xf',self.path],capture_output=True,text=True)
                try:
                    s=results.stdout.split()
                except:
                    return None
                ...
                self.extracted_path=pl.Path(s[0])
                return str(self.path.parent / self.extracted_path)
            else:
                return None
